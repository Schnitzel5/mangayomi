...
use crate::api::colorizer::{types::AppConfig, utils::compute_integral_image};
...
pub async fn colorize(
    ...
    if let Ok(()) = receiver.await? {
        ...
        process_result(&device, &queue, result, width, height, params_buffer).await
    } else {
...
async fn process_result(
    ...
    let spatially_averaged = compute_integral_image(&img);
